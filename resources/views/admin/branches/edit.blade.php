...
@section('content')
    <h1 class="font-serif text-2xl font-semibold text-trust-900">Edit branch</h1>
    <form method="POST" action="{{ route('admin.branches.update', $branch) }}" class="mt-8 max-w-xl space-y-4">
        @csrf
        @method('PUT')
        <div>
            <label for="label" class="block text-sm font-medium text-stone-700">Label</label>
            <input type="text" name="label" id="label" value="{{ old('label', $branch->label) }}" required
        ...
        <div>
            ...
            <textarea name="address_lines" id="address_lines" rows="5" required
                      class="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 shadow-sm focus:border-trust-500 focus:outline-none focus:ring-1 focus:ring-trust-500">{{ old('address_lines', $branch->address_lines) }}</textarea>
        </div>
        <div class="flex items-center gap-2">
            <input type="checkbox" name="is_main" id="is_main" value="1" {{ old('is_main', $branch->is_main) ? 'checked' : '' }}
